...
module.exports = [ 'List', 'lazy', 'typeOf', 'error', 'Thunk', 'tuple', 'Stream', 'emptyList',
  function (List, lazy, typeOf, error, Thunk, tuple, Stream, emptyList) {
  ...
  function genInitList(xs) {
    if (xs.tail().isEmpty()) {
      return emptyList();
    }
    return new List(xs.head(), new Thunk(function () {
      return genInitList(xs.tail());
    }));
  }
